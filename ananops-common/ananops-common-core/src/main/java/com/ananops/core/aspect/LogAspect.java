...
public class LogAspect {
	 ...
	 * Log annotation.
	 */
	@Pointcut("@annotation(com.ananops.core.annotation.LogAnnotation)")
	...
	private void handleLog(final JoinPoint joinPoint, final Object result) {
		...
		try {
			...
			threadLocal.remove();
			taskExecutor.execute(() -> this.restTemplate.postForObject("http://ananops-provider-uac/auth/saveLog", operationLogDto, Integer.class));
		} catch (Exception ex) {
